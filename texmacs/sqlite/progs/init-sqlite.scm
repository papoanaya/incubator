...
;; DESCRIPTION : Initialize sqlite plugin
...
(define (sqlite-serialize lan t)
  (import-from (utils plugins plugin-cmd))
  (with u (pre-serialize lan t)
    (with s (texmacs->verbatim (stree->tree u))
      (string-append  s  "\n"))))

;;  (:require (url-exists-in-path? "tm_sqlite"))
;;  (:launch "tm_sqlite --texmacs")
;;  (:require (url-exists-in-path? "sqlite"))
;;  (:serializer ,sqlite-serialize)
;;  (:tab-completion #t)

(plugin-configure sqlite
  (:require (url-exists-in-path? "sqlite3"))
  (:launch "gosqlite.nlsp")
  (:session "Sqlite"))
